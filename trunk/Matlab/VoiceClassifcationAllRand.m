...
y_rand = randperm(size(mfcc_y, 1));
mfcc_dmfcc_y = mfcc_y(y_rand, :); %Randomize feature set samples
...
z_rand = randperm(size(mfcc_z, 1));
mfcc_dmfcc_z = mfcc_z(z_rand, :); %Randomize feature set samples
...
w_rand = randperm(size(mfcc_w, 1));
mfcc_dmfcc_w = mfcc_w(w_rand, :); %Randomize feature set samples
...
for UseClassificationMethod = 0:4
...
mfcc_yt = mfcc_func(yt,fs,n,inc); % Computes cepstrum MFCC features voice 1
yt_rand = randperm(size(mfcc_yt, 1));
mfcc_dmfcc_yt = mfcc_yt(yt_rand, :); %Randomize feature set samples

wt = stereo2_test(start3_test:end3_test); % Select samples at other position in recording (Voice 2)
mfcc_wt = mfcc_func(w,fs,n,inc);
wt_rand = randperm(size(mfcc_wt, 1));
mfcc_dmfcc_wt = mfcc_wt(wt_rand, :); %Randomize feature set samples
