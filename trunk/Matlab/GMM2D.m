...
axis([xi xf yi yf])
title('GMM training data V1(red), V2(green), S(blue)');
...
title('GMM train+test used for validation');
...
for k = 1:mix.ncentres
    ...
    covar = [mix.covars(k,1) 0; 0 mix.covars(k,2)];
...
plot(pC_v1(2,:), 'g');
plot(pC_v1(3,:), 'b');
title('GMM test if V1 belongs to GM1(red), GM2(blue), GM3(green)');
...
plot(pC_v2(2,:), 'g');
plot(pC_v2(3,:), 'b');
title('GMM test if V2 belongs to GM1(red), GM2(blue), GM3(green)');
